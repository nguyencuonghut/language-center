...
function destroyInvoice() {
  ...
  router.delete(route('manager.invoices.destroy', props.invoice.id), { preserveScroll: true })
}
...
function savePayment() {
  ...
  router.post(route('manager.invoices.payments.store', props.invoice.id), {
...
function saveItem() {
  ...
  if (editMode.value) {
    router.put(route('manager.invoices.items.update', { invoice: props.invoice.id, item: itemForm.id }), payload, {
      preserveScroll: true,
      onFinish: () => { itemForm.saving = false },
      onSuccess: () => { showItemDialog.value = false }
    })
  } else {
    router.post(route('manager.invoices.items.store', props.invoice.id), payload, {
      preserveScroll: true,
      onFinish: () => { itemForm.saving = false },
      onSuccess: () => { showItemDialog.value = false }
    })
...
function deleteItem(item) {
  ...
  router.delete(route('manager.invoices.items.destroy', { invoice: props.invoice.id, item: item.id }), {
    preserveScroll: true
  })
}
...
<template>
  ...
  <div class="mb-3 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
    ...
    <div class="flex flex-wrap items-center gap-2">
      ...
      <Link :href="route('manager.invoices.index')" class="px-3 py-1.5 rounded-lg border border-slate-300 dark:border-slate-600 hover:bg-slate-100 dark:hover:bg-slate-800">
        ← Danh sách
      </Link>
    </div>
  </div>
