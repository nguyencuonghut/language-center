...
<template>
  <Head title="Quản lý điểm danh - Admin" />

  <h1 class="text-xl md:text-2xl font-heading font-semibold mb-3">Quản lý điểm danh - Admin</h1>
  ...
  <div class="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-2">
    <DataTable :value="props.sessions?.data ?? []" :paginator="true"
      ...
      <Column header="" style="width:160px">
        <template #body="{ data }">
          <Link :href="route('admin.attendance.show', { session: data.id })"
